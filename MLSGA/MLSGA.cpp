...
int main()
{
	...
	for (short iGA_mode = 0; iGA_mode < GA_mode.size(); iGA_mode++)								//loop for different hybrid modes
	{
		...
		for (int i_coevol = 0; i_coevol < coevol_amt; i_coevol++)
			Crossover_vect.push_back(Cross_Type(cross_ix, MODE_vector[i_coevol]));
		std::vector<mutation<short>*> Mutation_vect;	//Mutation initialisation
		for (int i_coevol = 0; i_coevol < coevol_amt; i_coevol++)
			Mutation_vect.push_back(Mut_Type(mut_ix, MODE_vector[i_coevol]));
		selection<individual>* Selection = Select_Type(select_ix);								//Selection initialisation
		...
		for (short MLS = n_MLS_b; MLS <= n_MLS_e; MLS++)												//loop for different MLS types
		{
			...
			if (MLS != 1 && MLS != 2 && MLS != 3 && MLS != 7 && MLS!=0)
				continue;
			for (n_col = n_col_b; n_col <= n_col_e; n_col += 7)										//loop for different col size
			{
									...
									for (float C_Prob = cross_prob_min; C_Prob < cross_prob_max + 0.001f; C_Prob += cross_prob_step)			//Loop for different crossover probabilities
									{
										...
										//for (float M_Prob = 1.f; M_Prob <= 1.f; M_Prob *= 10.f)
										{
											...
											for (int iRun = 0; iRun < n_runs; iRun++)														//loop for number of runs- Last loop
											{
												...
												//initialise values for penalty based constraints
												if (PENALTY_BASED_CONSTRAINTS)
													Pen_const::Initialise(Func[0].Cons(), n_col, Func[0].Objs());
												...
												for (short iCol = 0; iCol < col.size(); iCol++)
												{
													...
													if (PENALTY_BASED_CONSTRAINTS)
														Pen_const::Initialise_Col(col[iCol].Size_Show(), col[iCol].Index_Show() - 1);
												...
												for (int iGen = 1; iGen < GA_standard.Max_Gen(); iGen++)	//loop for number of generations
												{
													...
													for (short iCol = 0; iCol < n_col; iCol++)											//Loop for collectives
													{
														...
														if (MODE == "Normal")
														{
															...
															if (PENALTY_BASED_CONSTRAINTS)
																Pen_const::Fitness_Recalc(col_temp, true);
