...
const std::string MOEAD_weight_generator{ "Uniform" };	//Default "Uniform"	//Define how the MOEA/D generate weight vectors; "File" - get from file in \Input\MOEADWeight; "Sobol" - generated by sobol - not implemented properly; "Uniform" - uniform method from MSF/PSF and mine			
...
/*GA parameters - MTS*/
const int MTS_LocalSearch_test_amount = 1;	//Default 1			//How many times each local search is tested each generation
const int MTS_LocalSearch_amount = 1;	//Default 1				//How many local searches are made each generation
...
const int MTS_PF_refine_size = 1000;	//Default 1000			//Size of the PF after normal refining, when PF is too big (not the final one) for MTS only

/*GA parameters - BCE*/
//const int BCE_PC_capacity = 600;								//maximum size of the PC population - not used, now it is the size of the current population
const short BCE_mode = 2;	//Default 2							//setting variation in the NPC and PC evolution;  1: SBX crossover, 2: DE
...
const int PF_size = 1000;  //Default 1000						//Output size of the calculated PF
const int PF_refine_size = 1000;	//Default 1000				//Size of the PF after normal refining, when PF is too big (not the final one)
const unsigned short PF_res = 6; //Default 6					//Pareto front search resolution
...
const float cons_check_param = -0.000001;	//Default -0.000001 //When constrain is violated for IGD check
...
/*Output*/
const std::string real_PF_out = "PF_Real";	//Default "PF_Real"	//File name for real Pareto Front
