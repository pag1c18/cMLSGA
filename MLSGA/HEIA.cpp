...
void HEIA::DE_Update(std::vector<individual> & DEpop, collective & col)
{
	...
	for (int i = 0; i < DEpop.size(); i++)
	{
		...
		if (PENALTY_BASED_CONSTRAINTS)
		{
			Pen_const::Fitness_Recalc(offspring,col_ix);
			if (DEpop[i].Cons_Viol_Show() != offspring.Cons_Viol_Show())
				Pen_const::R_f_update(offspring.Cons_Viol_Show(), col_ix);
		}
		nfes++;
		DEpop[i] = offspring;
	}
...
void HEIA::SBX_Update(std::vector<individual> & SBXpop, collective & col)
{
	...
	for (int i = 0; i < SBXpop.size(); i++)
	{
		...
		{
			Pen_const::Fitness_Recalc(offspring, col_ix);
			if (SBXpop[i].Cons_Viol_Show() != offspring.Cons_Viol_Show())
				Pen_const::R_f_update(offspring.Cons_Viol_Show(), col_ix);
		}
		nfes++;
		SBXpop[i] = offspring;
...
void HEIA::Archive_Update(std::vector<individual> & merge_pop, int popsize, int clonesize, std::vector<short> fit_indexes)
{
	...
	if (PENALTY_BASED_CONSTRAINTS)
		Pen_const::Fitness_Recalc(solution_union, col_ix,false);
